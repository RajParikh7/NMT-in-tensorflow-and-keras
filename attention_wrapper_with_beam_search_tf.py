...
tgt_sos_id=tf.fill([batch_size], 1)
tgt_eos_id=0
...
#initial_state = attn_cell.zero_state( batch_size = batch_size , dtype=tf.float32 )
#initial_state = initial_state.clone(cell_state = encoder_state)
input_len_vec=tf.constant(input_len_vec)
attention_states = tf.transpose(encoder_outputs, [1, 0, 2])
encoder_outputs_t = tf.contrib.seq2seq.tile_batch(attention_states, multiplier=10)
input_len_vec_t = tf.contrib.seq2seq.tile_batch(input_len_vec, multiplier=10)
...
initial_state1 = decoder_cell.zero_state(batch_size*beam_width, tf.float32).clone(
